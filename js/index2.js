import {saveTask, getTasks, onGetTasks, deleteTask, getTask, updateTask, getUser, LibretasFre2 } from './firebase.js'
...
const tContInsp = document.getElementById('taskcontainerinsp');
   console.log("los datos del inspector, el conductor buscado es  : " +  IdBUsInspUPP);
...
var datosLibreta =doc.data();
  ...
  <h3 class="h5">N° de libretas : ${datosLibreta.LibIdentificador}</h3>
  <p> Categoria de libreta : ${datosLibreta.LibCategoria}</p>
  <p>Nombre del conductor : ${datosLibreta.LibName}</p>
  <p>Cedula del conductor : ${datosLibreta.LibCedula}</p>
  <p>Vencimiento de la libreta : ${datosLibreta.LibVto}</p>
  <p>Observaciones de la libreta : ${datosLibreta.LibCategoria}</p>
  <p id="Lib-Ptos-Insp">Los puntos que tiene el usuario son : ${datosLibreta.LibPuntos} </input>
  /* <input id="Lib-Ptos-Insp" value=${datosLibreta.LibPuntos} disabled> </input> */
  <p>Libreta habilitada : ${datosLibreta.LibStatus}</p>
  <br/>
  
  <button class='btn-EditIns' data-id="${doc.id}">Actualizars</button>
  <div>
